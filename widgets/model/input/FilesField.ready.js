const MozzlerFilesArrayIFFY = function ($mozzlerFieldWrapper) {

    this.$mozzlerFieldWrapper = $mozzlerFieldWrapper;
    this.$addFileButton = this.$mozzlerFieldWrapper.find(".mozzler-files-field-add-file");
    this.$fileArrayWrapper = this.$mozzlerFieldWrapper.find(".mozzler-files-filepond-array-entries-wrapper");
    this.$inputField = this.$mozzlerFieldWrapper.find("input[type=hidden]");

    let that = this;
    let mozzlerFilesArray = {}; // Need an elementId and then the Filepond reference


    this.$addFileButton.on('click tap', function () {
        this.addFile();
    });
